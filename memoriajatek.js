$('document').ready(function(){
    ...
    $('img').click(function(){
        if (clickDisabled || $(this).hasClass('disabled')){
            return;
        }
        ...
        if(clickCounter % 2 == 0){
            ...
            if(tomb[clicks[clicks.length-1]]!=tomb[clicks[clicks.length-2]] && $("img[src='web.png']").length % 2 == 0){
                $('.clear').text("Nem egyeznek a kártyák, visszafordítjuk őket!");
                setTimeout(function() {
                    $('#' + clicks[clicks.length-1]).attr('src', 'web.png');}, 1000);
                setTimeout(function() {
                    $('#' + clicks[clicks.length-1]).removeClass('disabled');}, 1000);
                
                setTimeout(function() {
                    $('#' + clicks[clicks.length-2]).attr('src', 'web.png');}, 1000);
                setTimeout(function() {
                    $('#' + clicks[clicks.length-2]).removeClass('disabled');}, 1000);
                ...
                $('#' + clicks[clicks.length-1]).off('click');
                $('#' + clicks[clicks.length-2]).off('click');
            }
